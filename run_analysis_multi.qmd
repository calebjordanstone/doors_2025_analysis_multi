...
# KG: need code to add required folders if they don't exist - e.g. 'res'
# KG: can be handy to put a file path diagram above, so people know what structure they should have for things to work
...
# KG: I reckon delete this code block to avoid duplicate code.
...
multi_cond <- list(
  ...
  none_other = rep(c(0, 1, -1), times=4), # KG: why not also none_neither, if going this route?
...
G # look at G
# 1 & 4 are non-orthogonal # KG: these numbers don't match to the output of G, I get 1 & 2, 4 & 6, 5 & 7. can you update the comments?
...
# show results
res$contrasts
res$plot # KG: I only get one plot. Is that what you meant?
...
# KG: re the above - lets revisit the flexibility analysis after this one to check we're happy.
# KG: also, the first vs half thing was of interest to Shaily. Do you think its of interest to the paper we'd write here? Let's discuss
