...
First, get transition entropy (TE) scores per subject and session
...
TEs <- evt_multi[ses %in% c('training', 'testing'), # (ses=='training' & t > 320) | ses=='testing',
                 .(TEs=get_TE(door)),
...
Looks like TE is higher during training:
...
# # run t-test
# (ttst_TE <- pairwise_t_test(TEs, TEs ~ ses, paired=T))
# mdl_TE <- lmer(TEs ~ ses*train_type + (1|sub), data=TEs)
# 
# # save output
# dt <- data.table(ttst_TE)
# dt[, statistic := lapply(statistic, round, 2),
# ][, names(.SD) := lapply(.SD, round, 3), .SDcols=c('p', 'p.adj')]
# 
# # save result
# fln <- str_glue("res/ttest_exp-{exp}_TE.csv")
# write_csv(dt, fln)

```

#### Plot the TE scores per group by session
...
Now look at the relationship between TE and multitasking costs. Below I look at whether TE from training predicts the difference in setting errors between the 'other' and 'none' conditions, the difference in general errors between the 'neither' and 'none' conditions, and the difference in RT between the average of the 'other' and 'neither' conditions and the 'none' condition, for both the 'first' and 'subsequent' multitasking trials. I also look at the difference in multitasking costs between the first and subsequent sessions.
...
# merge data frames
# TEs <- merge(TEs, avg_multi_tst_diff, c('sub', 'ses'), all = T)
...
The above analysis produces 1 significant result: An effect in the Stable group for RT First Correct on subsequent multitasking trials. However, the p-value is only 0.025, so wouldn't survive correction for the 9 different models that I ran.

```{r}

summary(mdl_rt_diff_sub)$coefficients

ggplot(data=TEs[ses=='training'], aes(x=TEs, y=rt_first_cor_diff_subsequent, color=train_type)) + 
  geom_point() + 
...
Lastly, look at whether TE is associated with mindfulness:
